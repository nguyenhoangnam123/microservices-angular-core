...
export class RemarksListComponent implements OnInit, OnDestroy {
  ...
  loadRemarksList() {
    ...
    this.store.dispatch(
      new ProductRemarksPageRequested({
        page: queryParams,
  ...
  saveNewRemark() {
    ...
    this.componentSubscriptions = this.store.pipe(select(selectLastCreatedProductRemarkId)).subscribe(res => {
      if (!res) {
        return;
      }
