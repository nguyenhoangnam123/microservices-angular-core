...
public class OAuth2Properties {
    ...
    public static class WebClientConfiguration {
        ...
        /**
         * Holds the session timeout in seconds for non-remember-me sessions. After so
         * many seconds of inactivity, the session will be terminated. Only checked
         * during token refresh, so long access token validity may delay the session
         * timeout accordingly.
         */
         ...
         * then cookies will be set on {@code .your-domain.com}, such that they are also
         * valid for {@code app2.your-domain.com}.
         */
    ...
    public static class SignatureVerification {
        /**
         * Maximum refresh rate for public keys in ms. We won't fetch new public keys
         * any faster than that to avoid spamming UAA in case we receive a lot of
         * "illegal" tokens.
         */
        private long publicKeyRefreshRateLimit = 10 * 1000L;
        /**
         * Maximum TTL for the public key in ms. The public key will be fetched again
         * from UAA if it gets older than that. That way, we make sure that we get the
         * newest keys always in case they are updated there.
         */
