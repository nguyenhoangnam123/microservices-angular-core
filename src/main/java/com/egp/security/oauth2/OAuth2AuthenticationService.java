package com.egp.security.oauth2;
...
public class OAuth2AuthenticationService {
     ...
     * This avoids hammering UAA in case of several multi-threaded requests arriving
     * in parallel.
     */
     ...
     * Note that browsers typically send multiple requests in parallel which means
     * the access token will be expired on multiple threads. We don't want to send
     * multiple requests to UAA though, so we need to cache results for a certain
     * duration and synchronize threads to avoid sending multiple requests in
     * parallel.
     *
     * @param request       the request potentially holding the refresh token.
     * @param response      the response setting the new cookies (if refresh was
     *                      successful).
     * @param refreshCookie the refresh token cookie. Must not be null.
     * @return the new servlet request containing the updated cookies for relaying
     *         downstream.
     */
     ...
     * @return a RefreshGrantResult for that token. This will either be empty, if we
     *         are the first one to do the request, or contain some results already,
     *         if another thread already handled the grant for us.
     */
     ...
     * For example, the OAuth2 client won't checked them and they won't be relayed
     * to other services.
     *
