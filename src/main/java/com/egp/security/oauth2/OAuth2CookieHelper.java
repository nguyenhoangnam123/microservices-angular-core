...
import com.egp.config.oauth2.OAuth2Properties;
...
public class OAuth2CookieHelper {
     ...
     * non-persistent cookies, i.e. when remember me was not checked. The motivation
     * for this is that we want to throw out a user after a while if he's inactive.
     * We cannot do this via refresh token validity because that one is also used
     * for remember me.
     *
     ...
     * @throws InvalidTokenException if we cannot find the claim in the token or it
     *                               is of wrong type.
     */
     ...
     * @param domain   the domain for which the cookie is valid. If {@code null},
     ...
     * level domain.
     * <p>
     * For example, when sending a request to {@code app1.domain.com}, this returns
     * {@code .domain.com}.
     *
     * @param request the HTTP request we received from the client.
     * @return the top level domain to set the cookies for. Returns {@code null} if
     *         the domain is not under a public suffix (.com, .co.uk), e.g. for
     *         localhost.
     */
