...
public class RefreshTokenFilter extends GenericFilterBean {
    /**
     * Number of seconds before expiry to start refreshing access tokens so we don't
     * run into race conditions when forwarding requests downstream. Otherwise,
     * access tokens may still be valid when we check here but may then be expired
    ...
    /**
     * The {@link OAuth2AuthenticationService} is doing the actual work. We are just
     * a simple filter after all.
     */
     ...
     *
     * @param httpServletRequest  the servlet request holding the current cookies.
     *                            If no refresh cookie is present, then we are out
     *                            of luck.
     * @param httpServletResponse the servlet response that gets the new set-cookie
     *                            headers, if they had to be refreshed.
    ...
    public HttpServletRequest refreshTokensIfExpiring(HttpServletRequest httpServletRequest,
        ...
        if (mustRefreshToken(accessTokenCookie)) { // we either have no access token, or it is expired, or it is about
                                                   // to expire
            ...
            if (refreshCookie != null) {
                try {
                    newHttpServletRequest = authenticationService.refreshToken(httpServletRequest, httpServletResponse,
                            refreshCookie);
    ...
    /**
     * Check if we must refresh the access token. We must refresh it, if we either
     * have no access token, or it is expired, or it is about to expire.
     *
