...
import com.egp.web.filter.RefreshTokenFilter;
...
public class OAuth2AuthenticationServiceTest {
    ...
    private void mockInvalidPassword() {
        ...
        reqHeaders.add("Authorization", CLIENT_AUTHORIZATION); // take over Authorization header from client request to
                                                               // UAA request
    ...
    private void mockRefreshGrant() {
        ...
        // we must authenticate with the UAA server via HTTP basic authentication using
        // the browser's client_id with no client secret
    ...
    /**
     * If no refresh token is found and the access token has expired, then expect an
     * exception.
     */
    @Test
